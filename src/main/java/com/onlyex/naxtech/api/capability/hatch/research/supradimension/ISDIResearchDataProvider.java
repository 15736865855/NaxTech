 ...
 * 该接口为多方块结构提供了计算相关的功能。
 */
public interface ISDIResearchDataProvider {
    ...
    default int requestSDIRWUt(int sdirwut, boolean simulate) {
        Collection<ISDIResearchDataProvider> list = new ArrayList<>();
        list.add(this);
     ...
     */
    int requestSDIRWUt(int sdirwut, boolean simulate, @NotNull Collection<ISDIResearchDataProvider> seen);
     ...
     * seen 参数表示已经检查过的光学计算提供者。
     */
    int getMaxSDIRWUt(@NotNull Collection<ISDIResearchDataProvider> seen);
     ...
     * seen 参数表示已经检查过的光学计算提供者。
     */
    boolean canBridge(@NotNull Collection<ISDIResearchDataProvider> seen);
}
