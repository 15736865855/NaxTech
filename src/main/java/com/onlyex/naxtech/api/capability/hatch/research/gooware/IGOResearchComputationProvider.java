 ...
 * 该接口为多方块结构提供了计算相关的功能。
 */
public interface IGOResearchComputationProvider{
    ...
    default int requestGORWUt(int gorwut, boolean simulate) {
        Collection<IGOResearchComputationProvider> list = new ArrayList<>();
        list.add(this);
     ...
     */
    int requestGORWUt(int gorwut, boolean simulate, @NotNull Collection<IGOResearchComputationProvider> seen);
     ...
     * seen 参数表示已经检查过的光学计算提供者。
     */
    int getMaxGORWUt(@NotNull Collection<IGOResearchComputationProvider> seen);
     ...
     * seen 参数表示已经检查过的光学计算提供者。
     */
    boolean canBridge(@NotNull Collection<IGOResearchComputationProvider> seen);
}
