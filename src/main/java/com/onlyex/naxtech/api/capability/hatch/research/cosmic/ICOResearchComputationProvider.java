 ...
 * 该接口为多方块结构提供了计算相关的功能。
 */
public interface ICOResearchComputationProvider{
    ...
    default int requestCORWUt(int corwut, boolean simulate) {
        Collection<ICOResearchComputationProvider> list = new ArrayList<>();
        list.add(this);
     ...
     */
    int requestCORWUt(int corwut, boolean simulate, @NotNull Collection<ICOResearchComputationProvider> seen);
     ...
     * seen 参数表示已经检查过的光学计算提供者。
     */
    int getMaxCORWUt(@NotNull Collection<ICOResearchComputationProvider> seen);
     ...
     * seen 参数表示已经检查过的光学计算提供者。
     */
    boolean canBridge(@NotNull Collection<ICOResearchComputationProvider> seen);
}
