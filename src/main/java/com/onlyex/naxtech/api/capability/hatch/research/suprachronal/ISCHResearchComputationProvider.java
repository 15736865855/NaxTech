 ...
 * 该接口为多方块结构提供了计算相关的功能。
 */
public interface ISCHResearchComputationProvider{
    ...
    default int requestSCHRWUt(int schrwut, boolean simulate) {
        Collection<ISCHResearchComputationProvider> list = new ArrayList<>();
        list.add(this);
     ...
     */
    int requestSCHRWUt(int schrwut, boolean simulate, @NotNull Collection<ISCHResearchComputationProvider> seen);
     ...
     * seen 参数表示已经检查过的光学计算提供者。
     */
    int getMaxSCHRWUt(@NotNull Collection<ISCHResearchComputationProvider> seen);
     ...
     * seen 参数表示已经检查过的光学计算提供者。
     */
    boolean canBridge(@NotNull Collection<ISCHResearchComputationProvider> seen);
}
