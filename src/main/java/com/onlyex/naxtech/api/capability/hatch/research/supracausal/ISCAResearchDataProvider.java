 ...
 * 该接口为多方块结构提供了计算相关的功能。
 */
public interface ISCAResearchDataProvider {
    ...
    default int requestSCARWUt(int scarwut, boolean simulate) {
        Collection<ISCAResearchDataProvider> list = new ArrayList<>();
        list.add(this);
     ...
     */
    int requestSCARWUt(int scarwut, boolean simulate, @NotNull Collection<ISCAResearchDataProvider> seen);
     ...
     * seen 参数表示已经检查过的光学计算提供者。
     */
    int getMaxSCARWUt(@NotNull Collection<ISCAResearchDataProvider> seen);
     ...
     * seen 参数表示已经检查过的光学计算提供者。
     */
    boolean canBridge(@NotNull Collection<ISCAResearchDataProvider> seen);
}
