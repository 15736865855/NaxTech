...
public interface IResearchDataProvider {
    ...
    default int requestRWUt(int rwut, boolean simulate) {
        Collection<IResearchDataProvider> list = new ArrayList<>();
        list.add(this);
     ...
     */
    int requestRWUt(int rwut, boolean simulate, @NotNull Collection<IResearchDataProvider> seen);
     ...
     * seen 参数表示已经检查过的光学计算提供者。
     */
    int getMaxRWUt(@NotNull Collection<IResearchDataProvider> seen);
     ...
     * seen 参数表示已经检查过的光学计算提供者。
     */
    boolean canBridge(@NotNull Collection<IResearchDataProvider> seen);
}
