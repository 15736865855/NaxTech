package com.onlyex.naxtech.common.block.wood;
...
public class BlockPineLeaves extends BlockLeaves {
    ...
    public Item getItemDropped(@Nonnull IBlockState state, @Nonnull Random rand, int fortune) {
        return Item.getItemFromBlock(NTMetaBlocks.PINE_SAPLING);
    }
    ...
    @Override
    public boolean isOpaqueCube(@Nonnull IBlockState state) {//TODO 2
        if (!fancyLeaves()) {
    ...
    @Override
    public void getDrops(@Nonnull NonNullList<ItemStack> drops, @Nonnull IBlockAccess world, @Nonnull BlockPos pos, @NotNull IBlockState state, int fortune) {
        Random rand = world instanceof World ? ((World) world).rand : RANDOM;
        if (rand.nextDouble() <= .5D) {
            drops.add(NTMetaItems.PINE_CONE.getStackForm(GTValues.RNG.nextInt(3) + 1));
        }
    }
