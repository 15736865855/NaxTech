...
public class WorldGenPine extends WorldGenerator {
    ...
    public boolean grow(World world, BlockPos pos, Random random) {
        ...
        IBlockState woodBlock = NTMetaBlocks.PINE_LOG.getDefaultState().withProperty(NATURAL, true);
        IBlockState leaves = NTMetaBlocks.PINE_LEAVES.getDefaultState();
    ...
    public int getGrowHeight(@Nonnull World world, @Nonnull BlockPos pos) {
        ...
        if (baseBlock.isAir(baseState, world, below) ||
                !baseBlock.canSustainPlant(baseState, world, below, EnumFacing.UP, NTMetaBlocks.PINE_SAPLING) || (
                !world.isAirBlock(pos.up()) && world.getBlockState(pos.up()).getBlock() != NTMetaBlocks.PINE_SAPLING))
            return 0;
        int height = 1;
        pos = pos.up();
