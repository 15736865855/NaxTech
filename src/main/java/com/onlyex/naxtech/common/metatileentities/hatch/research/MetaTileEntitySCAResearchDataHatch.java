...
public class MetaTileEntitySCAResearchDataHatch extends MetaTileEntityMultiblockPart implements
                                            IMultiblockAbilityPart<ISCAResearchDataHatch>, ISCAResearchDataHatch {

    private final boolean isTransmitter;

    public MetaTileEntitySCAResearchDataHatch(ResourceLocation metaTileEntityId, boolean isTransmitter) {
    ...
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity tileEntity) {
        return new MetaTileEntitySCAResearchDataHatch(metaTileEntityId, isTransmitter);
    }
    ...
    @Override
    public int requestSCARWUt(int scarwut, boolean simulate, @NotNull Collection<ISCAResearchDataProvider> seenProviders) {
        ...
        if (isTransmitter()) {
            // 向多方块控制器发送请求RWUt值的请求
            if (multiblockController instanceof ISCAResearchDataProvider computationProvider) {
            ...
            ISCAResearchDataProvider computationProvider = getResearchNetProvider();
    ...
    @Override
    public int getMaxSCARWUt(@NotNull Collection<ISCAResearchDataProvider> seenProviders) {
        seenProviders.add(this);
        var multiblockController = getController();
    ...
    public boolean canBridge(@NotNull Collection<ISCAResearchDataProvider> seen) {
        ...
        if (isTransmitter()) {
            ...
            if (controller instanceof ISCAResearchDataProvider provider) {
            ...
            ISCAResearchDataProvider provider = getResearchNetProvider();
            if (provider == null) {
    ...
    @Nullable
    private ISCAResearchDataProvider getResearchNetProvider() {
    ...
    @Override
    public MultiblockAbility<ISCAResearchDataHatch> getAbility() {
    ...
    @Override
    public void registerAbilities(List<ISCAResearchDataHatch> abilityList) {
