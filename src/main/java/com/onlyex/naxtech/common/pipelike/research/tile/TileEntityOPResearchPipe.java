...
import com.onlyex.naxtech.common.pipelike.research.net.handler.OPResearchNetHandler;
...
public class TileEntityOPResearchPipe extends TileEntityPipeBase<ResearchPipeType, ResearchPipeProperties> {
    ...
    private void initHandlers() {
        ...
        for (EnumFacing facing : EnumFacing.VALUES) {
            handlers.put(facing, new OPResearchNetHandler(net, this, facing));
        }
        defaultHandler = new OPResearchNetHandler(net, this, null);
    }
    ...
    public <T> T getCapabilityInternal(Capability<T> capability, @Nullable EnumFacing facing) {
        ...
        if (capability == NTTileCapabilities.OP_CABABILITY_RESEARCH_PROVIDER) {
            if (world.isRemote) {
                return NTTileCapabilities.OP_CABABILITY_RESEARCH_PROVIDER.cast(clientComputationHandler);
            }

            if (handlers.isEmpty()) initHandlers();

            checkNetwork();
            return NTTileCapabilities.OP_CABABILITY_RESEARCH_PROVIDER
    ...
    public void checkNetwork() {
        if (defaultHandler != null) {
            ...
            if (defaultHandler.getNet() != current) {
                ...
                for (OPResearchNetHandler handler : handlers.values()) {
    ...
    public void transferDataFrom(IPipeTile<ResearchPipeType, ResearchPipeProperties> tileEntity) {
        ...
        if (getResearchPipeNet() == null)
            return;
        TileEntityOPResearchPipe pipe = (TileEntityOPResearchPipe) tileEntity;
