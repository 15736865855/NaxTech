...
import com.onlyex.naxtech.common.pipelike.research.net.handler.COResearchNetHandler;
...
public class TileEntityCOResearchPipe extends TileEntityPipeBase<ResearchPipeType, ResearchPipeProperties> {
    ...
    private void initHandlers() {
        ...
        for (EnumFacing facing : EnumFacing.VALUES) {
            handlers.put(facing, new COResearchNetHandler(net, this, facing));
        }
        defaultHandler = new COResearchNetHandler(net, this, null);
    }
    ...
    public <T> T getCapabilityInternal(Capability<T> capability, @Nullable EnumFacing facing) {
        ...
        if (capability == NTTileCapabilities.CO_CABABILITY_RESEARCH_PROVIDER) {
            if (world.isRemote) {
                return NTTileCapabilities.CO_CABABILITY_RESEARCH_PROVIDER.cast(clientComputationHandler);
            }

            if (handlers.isEmpty()) initHandlers();

            checkNetwork();
            return NTTileCapabilities.CO_CABABILITY_RESEARCH_PROVIDER
    ...
    public void checkNetwork() {
        if (defaultHandler != null) {
            ...
            if (defaultHandler.getNet() != current) {
                ...
                for (COResearchNetHandler handler : handlers.values()) {
    ...
    public void transferDataFrom(IPipeTile<ResearchPipeType, ResearchPipeProperties> tileEntity) {
        ...
        if (getResearchPipeNet() == null)
            return;
        TileEntityCOResearchPipe pipe = (TileEntityCOResearchPipe) tileEntity;
