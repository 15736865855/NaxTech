...
import com.onlyex.naxtech.common.pipelike.research.net.handler.SCAResearchNetHandler;
...
public class TileEntitySCAResearchPipe extends TileEntityPipeBase<ResearchPipeType, ResearchPipeProperties> {
    ...
    private void initHandlers() {
        ...
        for (EnumFacing facing : EnumFacing.VALUES) {
            handlers.put(facing, new SCAResearchNetHandler(net, this, facing));
        }
        defaultHandler = new SCAResearchNetHandler(net, this, null);
    }
    ...
    public <T> T getCapabilityInternal(Capability<T> capability, @Nullable EnumFacing facing) {
        ...
        if (capability == NTTileCapabilities.SCA_CABABILITY_RESEARCH_PROVIDER) {
            if (world.isRemote) {
                return NTTileCapabilities.SCA_CABABILITY_RESEARCH_PROVIDER.cast(clientComputationHandler);
            }

            if (handlers.isEmpty()) initHandlers();

            checkNetwork();
            return NTTileCapabilities.SCA_CABABILITY_RESEARCH_PROVIDER
    ...
    public void checkNetwork() {
        if (defaultHandler != null) {
            ...
            if (defaultHandler.getNet() != current) {
                ...
                for (SCAResearchNetHandler handler : handlers.values()) {
    ...
    public void transferDataFrom(IPipeTile<ResearchPipeType, ResearchPipeProperties> tileEntity) {
        ...
        if (getResearchPipeNet() == null)
            return;
        TileEntitySCAResearchPipe pipe = (TileEntitySCAResearchPipe) tileEntity;
