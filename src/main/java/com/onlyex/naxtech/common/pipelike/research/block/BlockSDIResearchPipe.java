...
import com.onlyex.naxtech.common.pipelike.research.item.ItemBlockSDIResearchPipe;
...
public class BlockSDIResearchPipe extends BlockPipe<ResearchPipeType, ResearchPipeProperties, WorldResearchPipeNet> {

    private final ResearchPipeType pipeType;
    private final ResearchPipeProperties properties;

    public BlockSDIResearchPipe(@NotNull ResearchPipeType pipeType) {
    ...
    protected Pair<TextureAtlasSprite, Integer> getParticleTexture(@NotNull World world, BlockPos blockPos) {
        return ResearchPipeRenderer.INSTANCE.getParticleTexture((TileEntitySDIResearchPipe) world.getTileEntity(blockPos));
    }
    ...
    public TileEntityPipeBase<ResearchPipeType, ResearchPipeProperties> createNewTileEntity(boolean supportsTicking) {
        return new TileEntitySDIResearchPipe();
    }
    ...
    public ResearchPipeProperties createItemProperties(@NotNull ItemStack itemStack) {
        if (itemStack.getItem() instanceof ItemBlockSDIResearchPipe pipe) {
            return ((BlockSDIResearchPipe) pipe.getBlock()).properties;
        }
        return null;
    }
    ...
    public ResearchPipeType getItemPipeType(@NotNull ItemStack itemStack) {
        if (itemStack.getItem() instanceof ItemBlockSDIResearchPipe pipe) {
            return ((BlockSDIResearchPipe) pipe.getBlock()).pipeType;
        }
        return null;
    }
    ...
    public boolean canPipesConnect(IPipeTile<ResearchPipeType, ResearchPipeProperties> selfTile, EnumFacing side,
        ...
        return selfTile instanceof TileEntitySDIResearchPipe && sideTile instanceof TileEntitySDIResearchPipe;
    }
    ...
    public boolean isHoldingPipe(EntityPlayer player) {
        ...
        return stack != ItemStack.EMPTY && stack.getItem() instanceof ItemBlockSDIResearchPipe;
    }
