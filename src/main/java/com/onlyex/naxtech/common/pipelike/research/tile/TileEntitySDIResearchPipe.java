...
import com.onlyex.naxtech.common.pipelike.research.net.handler.SDIResearchNetHandler;
...
public class TileEntitySDIResearchPipe extends TileEntityPipeBase<ResearchPipeType, ResearchPipeProperties> {
    ...
    private void initHandlers() {
        ...
        for (EnumFacing facing : EnumFacing.VALUES) {
            handlers.put(facing, new SDIResearchNetHandler(net, this, facing));
        }
        defaultHandler = new SDIResearchNetHandler(net, this, null);
    }
    ...
    public <T> T getCapabilityInternal(Capability<T> capability, @Nullable EnumFacing facing) {
        ...
        if (capability == NTTileCapabilities.SDI_CABABILITY_RESEARCH_PROVIDER) {
            if (world.isRemote) {
                return NTTileCapabilities.SDI_CABABILITY_RESEARCH_PROVIDER.cast(clientComputationHandler);
            }

            if (handlers.isEmpty()) initHandlers();

            checkNetwork();
            return NTTileCapabilities.SDI_CABABILITY_RESEARCH_PROVIDER
    ...
    public void checkNetwork() {
        if (defaultHandler != null) {
            ...
            if (defaultHandler.getNet() != current) {
                ...
                for (SDIResearchNetHandler handler : handlers.values()) {
    ...
    public void transferDataFrom(IPipeTile<ResearchPipeType, ResearchPipeProperties> tileEntity) {
        ...
        if (getResearchPipeNet() == null)
            return;
        TileEntitySDIResearchPipe pipe = (TileEntitySDIResearchPipe) tileEntity;
